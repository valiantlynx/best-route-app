...
for route in traffic_data:
    # Example feature: convert road path to a simple numeric feature
    ...
    # Feature vector: road and departure time
    features = road_feature * 0.7 + departure_time * 0.3  # Example feature weighting
    xs.append((features, route['depature']))  # Store both features and departure time
    # Observed outcome: travel time
    ys.append(travel_time)
...
# Define the model function (linear model)
def f(x, theta):
    a, c = theta
    return a * x + c


# Number of iterations for the Fortuna algorithm
N = 100000
best_loss = float('inf')
best_theta = None
best_loss_log = []
...
def find_best_route(input_departure, traffic_data, best_theta):
    ...
    for route in traffic_data:
        ...
        route_feature = hash(route['road']) % 100 * 0.7 + datetime.strptime(route['depature'], "%H:%M").hour * 0.3
